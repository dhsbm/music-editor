...
<template>
  ...
  >
    <Title
      ...
      :start="true"
    ></Title>
    <div v-if="patternEditorData.clone" class="content">
      <div class="container">
        <div class="rangeSetting">
          <span class="name">响度:</span>
          <input v-model.number="patternEditorData.clone.volume" type="range" max="100" min="0" step="1" />
          <span class="num">{{ patternEditorData.clone.volume }}</span>
          <br />
        </div>
      </div>
      <div class="save">
        <button @click="savePattern">保存</button>
      </div>
    </div>
    <div v-else class="content">
      <div class="tip">请先绑定音谱</div>
    </div>
  </div>
</template>
